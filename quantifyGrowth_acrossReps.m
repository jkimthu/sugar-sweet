%% quantify growth across replicates
%
...
%
%          plot compares experiments  from two conditions:
%          (i) wild-type YFP cells, (ii) mutant CFP cells (delta-glgG)
%
...
dates = {'2019-02-06','2019-02-19','2019-02-25'};
counter = 0;
...
for rep = 1:length(dates)
    ...
    title(strcat('growth in pulsing nutrient: YFP wt vs CFP delta-glgP'))
    xlabel('time (hr)')
    ylabel(strcat('growth rate: (', specificGrowthRate,')'))
    axis([0.9,8.1,-0.03,0.35])
...
save('glycogen_mu_vs_time.mat','compiled_yfps','compiled_cfps','times','dates')

figure(1)
plotName = strcat('glycogen-mu-v-time-fig1-individualReplicates');
saveas(gcf,plotName,'epsc')
close(gcf)

figure(2)
plotName = strcat('glycogen-mu-v-time-fig2-replicateStats');
saveas(gcf,plotName,'epsc')
close(gcf)
